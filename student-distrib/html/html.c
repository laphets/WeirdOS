...
HtmlAttrib *html_new_element_attrib(enum HtmlAttribKey key, char *key_name, const char* value, size_t length) {
	HtmlAttrib *attrib;
	if(!(attrib = kmalloc(sizeof(HtmlAttrib))))
		return NULL;
...
HtmlElement *html_new_element(HtmlTag tag, char *tag_name, HtmlAttrib *attrib, HtmlElement *child, HtmlElement *sibling, char *text) {
	HtmlElement *elem;
	if(!(elem = kmalloc(sizeof(HtmlElement))))
		return NULL;
...
HtmlParseState *html_parse_begin() {
	...
	if(!(state = kmalloc(sizeof(HtmlParseState))))
		return NULL;
	...
	if(!(state->document = kmalloc(sizeof(HtmlDocument))))
		goto error;
...
void *html_print_dom_element(HtmlElement *element, int level) {
	...
	while(element) {
		...
		if(element->text) {
			...
			kprintf("element: %s ", html_tag[element->tag]);
			attrib = element->attrib;
			if (attrib) {
				printf("[ ");
				do{
					if (attrib->key == HTML_ATTRIB_UNKNOWN) {
						kprintf("%s=\"%s\" ", attrib->key_name, attrib->value);
					} else {
						kprintf("%s=\"%s\" ", html_attrib[attrib->key], attrib->value);
					}
					attrib = attrib->next;
				}while(attrib);
				kprintf("]");
...
void *html_free_element(HtmlElement *element) {
	...
	while(element) {
		...
		kfree(element->attrib);
		if(element->tag_name) {
			kfree(element->tag_name);
		}
		if(element->text) {
			kfree(element->text);
		}
